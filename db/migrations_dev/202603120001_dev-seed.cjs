...
module.exports.up = (pgm) => {
  pgm.sql(`
    ...
    SELECT
      (CASE current_schema()
        WHEN 'operational_grace'          THEN 'a0000000-0000-4000-a000-00000000000'
        WHEN 'operational_grace_training' THEN 'b0000000-0000-4000-b000-00000000000'
      END || seq)::uuid,
      property_uuid::uuid
    FROM (VALUES
      ('1', '${PROP1_UUID}'),
...
module.exports.down = (pgm) => {
  pgm.sql(`
    ...
    WHERE uuid IN (
      ...
      'b0000000-0000-4000-b000-000000000002'::uuid,
      'b0000000-0000-4000-b000-000000000003'::uuid,
      'b0000000-0000-4000-b000-000000000004'::uuid,
      'b0000000-0000-4000-b000-000000000005'::uuid
    )
  `);
};
